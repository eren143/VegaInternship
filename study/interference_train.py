...
g = df.groupby(['interface','modelName'])

#enter interface and model to plot
interface_model = (0,'DLink DIR-853/ET')

score_column = g.get_group((interface_model[0],interface_model[1]))['max_intf_score']
predict_column = g.get_group((interface_model[0],interface_model[1]))['prob_isoR_smoothed']
train_column = g.get_group((interface_model[0],interface_model[1]))['ticket_prob']
